...
def opentext():
    proc = subprocess.Popen(['pluma', '/home/extra_info.txt']) # adress of a file to open with extra info
...
fonts = cycle((('Helvetica', '9'),('Helvetica', '9'),('Helvetica', '9'))) # font
# put there =>font=next(fonts)


#button color
dataifusp=int(31-float(data[8:10]))    # Some data  minus the current day
dataTese=int(29-float(data[8:10]))
dataufrn=int(27-float(data[8:10]))
dataufrpe=int(14+22-float(data[8:10]))
...
class App:
    def __init__(self, master):
        ...
        Button(fm, text='Pos-Doc 31/08->'+str(int(31-float(data[8:10]))), command=opentext,bg = colorbutton("snow3",dataifusp)).pack(side=TOP, fill=X, expand=YES)
        Button(fm, text='Classes', fg="orange",bg = "black").pack(side=TOP,    fill=X, expand=YES)

        Button(fm, text='Date 27->'+str([int(27-float(data[8:10])),int(15+22-float(data[8:10])),int(20+22-float(data[8:10]))]), command=opentext, fg="black" ,bg = colorbutton("coral",dataufrn) ,font=next(fonts)).pack(side=TOP, fill=X, expand=YES) 

        Button(fm, text='Date II->'+str([int(14+22-float(data[8:10]))]), command=opentext, fg="black" ,bg = colorbutton("turquoise1",dataufrpe)).pack(side=TOP, fill=X, expand=YES) 

        Button(fm, text='Date III', relief= SUNKEN).pack(side=TOP, fill=X, expand=YES)
        #Button(fm, text='Pos-Doc uaf.ufcg 21 june').pack(side=TOP, fill=X, expand=YES)

        Button(fm, text='Date IV', command=opentext).pack(side=TOP, fill=X, expand=YES)
#        Button(fm, text='Concursos IF',  fg="black" ).pack(side=TOP, fill=X, expand=YES)
   #     Button(fm, text='Concurso PB', command=opentext, fg="black" ).pack(side=TOP, fill=X, expand=YES)
        ...
        Button(fm, text= 'Date V',bg = "seashell2" ,font=next(fonts)).pack(side=LEFT) #\u03a9 h^2
#        Button(fm, text= u'<\u03C3v^2>').pack(side=LEFT) #\u03a9 h^2
        Button(fm, text='Date VI',bg = "seashell3").pack(side=LEFT)
        #Button(fm, text='Tese', relief=FLAT).pack(side=TOP, fill=X, expand=YES)
        Button(fm, text='Date VII',bg ='dark orange' ,font=next(fonts)).pack(side=RIGHT, fill=X, expand=YES)
